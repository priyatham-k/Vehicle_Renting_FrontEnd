...
function Ownerdashboard() {
  ...
  useEffect(() => {
    const fetchVehicles = async () => {
      try {
        const customerVehiclesResponse = await axios.get("http://localhost:3001/api/customers/vehicles");
    ...
    const fetchRentals = async () => {
      try {
        const rentalsResponse = await axios.get("http://localhost:3001/api/owners/rentals");
  ...
  const handleDeleteVehicle = async (vehicleId) => {
    ...
    try {
      await axios.delete(`http://localhost:3001/api/owners/deleteVehicle/${vehicleId}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        const response = await axios.post("http://localhost:3001/api/owners/vehicles", newVehicle);
      ...
      <div id="wrapper">
        <ul className="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
          ...
          <li className={`nav-item ${activeSection === "dashboard" ? "active" : ""}`}>
            <a className="nav-link" onClick={() => setActiveSection("dashboard")}>
          ...
          <li className={`nav-item ${activeSection === "vehicles" ? "active" : ""}`}>
            <a className="nav-link" onClick={() => setActiveSection("vehicles")}>
              <span>Vehicles</span>
              <i className="fas fa-car" style={{ marginLeft: "12px", fontSize: "1.1rem" }}></i>
            </a>
          </li>
          ...
          <li className={`nav-item ${activeSection === "rentals" ? "active" : ""}`}>
            <a className="nav-link" onClick={() => setActiveSection("rentals")}>
              <span>Rentals</span>
              <i className="fas fa-clipboard-list" style={{ marginLeft: "12px", fontSize: "1.1rem" }}></i>
            </a>
          </li>
              ...
              <div className="container-fluid">
                <div className="row">
                  <div className="col-12 ">
                    <button onClick={() => setShowModal(true)} className="btn btn-primary mb-3 float-right mr-4">
                      Add Vehicle
                    </button>
                  </div>
                  {error ? (
                          ...
                          <div className="row g-0">
                            ...
                            <div className="col-md-8">
                              <div className="card-body d-flex flex-column">
                                ...
                                <p className="card-insurance">Insurance Cost: {vehicle.insurance}</p>
                                <p className="card-price">Price per day: ${vehicle.dailyPrice}</p>
                                <div className="book-btn mt-auto" onClick={() => handleDeleteVehicle(vehicle._id)}>
                ...
                <form onSubmit={handleFormSubmit}>
                  ...
                  <div className="form-row">
                    <div className="form-group col-md-6">
                      ...
                      <input
                        type="number"
                        className={`form-control ${formErrors.insurance && "is-invalid"}`}
                        id="insurance"
                        name="insurance"
                        ...
                        required
                      />
                      {formErrors.insurance && <div className="invalid-feedback">{formErrors.insurance}</div>}
                    </div>
                    <div className="form-group col-md-6">
                      <label htmlFor="dailyPrice">Daily Price ($)</label>
                      <input
                        type="text"
                        className={`form-control ${formErrors.dailyPrice && "is-invalid"}`}
                        id="dailyPrice"
                        name="dailyPrice"
                        ...
                        required
                      />
                      {formErrors.dailyPrice && <div className="invalid-feedback">{formErrors.dailyPrice}</div>}
                    </div>
                  </div>
                  ...
                  <div className="form-row">
                    <div className="form-group col-md-6">
                      ...
                      <input
                        type="text"
                        className={`form-control ${formErrors.vinNumber && "is-invalid"}`}
                        id="vinNumber"
                        name="vinNumber"
                        ...
                        required
                      />
                      {formErrors.vinNumber && <div className="invalid-feedback">{formErrors.vinNumber}</div>}
                    </div>
                    <div className="form-group col-md-6">
